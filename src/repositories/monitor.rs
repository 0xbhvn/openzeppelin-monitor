...
mod tests {
	...
	async fn test_load_from_path_error_handling() {
		...
		match err {
			...
			_ => panic!("Expected RepositoryError::LoadError"),
		#[test]
		fn test_new_with_monitors_and_getters() {
		let mut monitors = HashMap::new();
		let m1 = MonitorBuilder::new().name("m1").build();
		let m2 = MonitorBuilder::new().name("m2").build();
		monitors.insert("m1".to_string(), m1.clone());
		monitors.insert("m2".to_string(), m2.clone());
		let repo = MonitorRepository::<NetworkRepository, TriggerRepository>::new_with_monitors(monitors.clone());
		assert_eq!(repo.get("m1"), Some(m1));
		assert_eq!(repo.get("m2"), Some(m2));
		assert_eq!(repo.get("nonexistent"), None);
		assert_eq!(repo.get_all(), monitors);
		}

		#[test]
		fn test_monitor_service_new_with_repository() {
		let mut monitors = HashMap::new();
		let m1 = MonitorBuilder::new().name("m1").build();
		let m2 = MonitorBuilder::new().name("m2").build();
		monitors.insert("m1".to_string(), m1.clone());
		...
		assert_eq!(service.get("m1"), Some(m1));
		assert_eq!(service.get("m2"), Some(m2));
